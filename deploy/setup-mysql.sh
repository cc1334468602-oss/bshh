...
# 4) 建库 + 建账号 + 授权 + 导入表结构（全部幂等，重复执行安全）
if $MYSQL_ADMIN -e "USE ${DB_NAME};" >/dev/null 2>&1; then
  green "  ✓ 数据库 ${DB_NAME} 已存在"
else
  $MYSQL_ADMIN -e "CREATE DATABASE IF NOT EXISTS ${DB_NAME} CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;"
  green "  ✓ 已创建数据库 ${DB_NAME}"
fi

# 应用账号与权限（固定密码 Bshh@2026，与 .env 保持一致）
$MYSQL_ADMIN <<SQL
...
FLUSH PRIVILEGES;
SQL
green "  ✓ 应用账号 ${DB_USER} 已就绪（localhost / 127.0.0.1，密码固定）"

# 导入表结构（CREATE TABLE IF NOT EXISTS，重复执行不会破坏数据）
